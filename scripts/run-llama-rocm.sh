...
SERVER="$ROOT/build/llama-rocm/bin/llama-server"
...
export LLAMA_BACKEND="${LLAMA_BACKEND:-rocm}"
...
echo "model=$MODEL_PATH sha256=$SHA backend=rocm bind=127.0.0.1:$PORT"
...
HIP_VISIBLE_DEVICES="${HIP_VISIBLE_DEVICES:-0}" ROCR_VISIBLE_DEVICES="${ROCR_VISIBLE_DEVICES:-0}" "$SERVER" "${ARGS[@]}" & SERVER_PID=$!
